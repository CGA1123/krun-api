#!/usr/bin/env bash
set -euo pipefail

cd "$(dirname "$0")"

API_URL="${API_URL:-http://localhost:9191}"
BASE_IMAGE="${BASE_IMAGE:-alpine:latest}"

# Build krun-run, krun-vmm, and vminit
echo "==> Building krun-run, krun-vmm, and vminit..."
go build -o krun-run ./cmd/krun-run/
go build -o krun-vmm ./cmd/krun-vmm/
codesign --sign - --entitlements entitlements.plist --force krun-vmm
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -o vminit ./cmd/vminit/

# Check if krun-api is actually running (verify we get valid JSON back)
if ! curl -sf "$API_URL/v1/machines" | python3 -c "import sys,json; json.load(sys.stdin)" 2>/dev/null; then
  echo ""
  echo "ERROR: krun-api server is not responding at $API_URL"
  echo ""
  echo "Start it in another terminal first:"
  echo "  cd $(pwd) && make && ./krun-api --vmm-path ./krun-vmm --vminit-path ./vminit --listen :9191"
  echo ""
  echo "Or set API_URL to point to your running instance:"
  echo "  API_URL=http://localhost:XXXX ./test-krun-run.sh"
  echo ""
  echo "Then re-run this script."
  exit 1
fi

echo "==> krun-api is running at $API_URL"
echo "==> Using base image: $BASE_IMAGE"

# Run krun-run with --image (pulls config via crane, server handles rootfs caching)
echo "==> Running: ./krun-run --api $API_URL --image $BASE_IMAGE --init ./vminit -n test-alpine"
./krun-run --api "$API_URL" --image "$BASE_IMAGE" --init ./vminit -n test-alpine

# Show current machines
echo ""
echo "==> Machines:"
curl -s "$API_URL/v1/machines" | python3 -m json.tool
